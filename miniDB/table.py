    ...
    def _select_where(self, return_columns, condition=None, distinct=False, order_by=None, desc=True, limit=None):
        ...
        if condition is not None:
                    ...
                    if (
                        ...
                        for i, j in enumerate(column):
                            if int(j) >= int(left_val) and int(j) <= int(right_val):
                                # stelnei stin grammi gia emfanish tis times pou epalitheuoun tin sunthiki
                                rows.append(i)
            ...
            # AND operator
                ...
                # get the intersection of the seperate conditions
                ...
                rows = [ind for ind, x in enumerate(
                    column) if get_op(operator2, x, value)]
